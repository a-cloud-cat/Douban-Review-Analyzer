...
if selected_page == "数据概览":
    ...
    st.markdown('<div class="main-title">情感预测</div>', unsafe_allow_html=True)
    st.markdown('<div style="font-size: 14px; color: #64748b; margin-bottom: 24px;">使用预训练模型或自定义模型进行情感分析预测</div>', unsafe_allow_html=True)
    
    # 选择预测模式
    prediction_mode = st.radio(
        "选择预测模式",
        ["预训练 BERT 模型（推荐）", "自定义训练模型（SVM/逻辑回归/决策树）"],
        horizontal=True,
        help="预训练模型效果更好，无需训练；自定义模型需要先训练"
    )
    
    if prediction_mode == "预训练 BERT 模型（推荐）":
        st.markdown("""
        <div style="background: #ecfdf5; border-radius: 8px; padding: 16px; margin-bottom: 24px;">
            <div style="font-weight: 600; color: #065f46; margin-bottom: 8px;">✅ 预训练 BERT 模型</div>
            <div style="font-size: 13px; color: #047857; line-height: 1.6;">
                使用 <code>uer/roberta-base-finetuned-dianping-chinese</code> 模型，专为中文评论情感分析优化，准确率约 92-95%
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        input_text = st.text_area("输入评论文本", placeholder="请输入一段图书评论，我来预测它的情感倾向...", height=100, key="bert_predict_input")
        
        if st.button("🔍 预测情感", use_container_width=True, key="bert_single_predict_btn"):
            if not input_text.strip():
                ...
                try:
                    ...
                    with st.spinner("正在分析..."):
                        ...
                        sentiment, confidence = analyzer.analyze(input_text)
                        
                        sentiment_color = "#10b981" if sentiment == '正面' else "#ef4444" if sentiment == '负面' else "#94a3b8"
                        st.markdown(f"""
                    ...
                    st.error(f"❌ 预测失败：{e}")
                    st.info("💡 提示：请先运行 `python scripts/download_bert_model.py` 下载模型")
        ...
        if st.button("🔄 测试示例评论", use_container_width=True, key="bert_batch_predict_btn"):
            try:
                from engines.clustering.sentiment_analyzer_v2 import get_bert_analyzer
                ...
                with st.spinner("正在分析..."):
                    ...
                    result_df['sentiment'] = result_df['sentiment'].apply(lambda x: f'<span style="color: {"#10b981" if x == "正面" else "#ef4444" if x == "负面" else "#94a3b8"}; font-weight: 500;">{x}</span>')
        ...
        <div style="background: #eff6ff; border-radius: 8px; padding: 16px; margin-bottom: 24px;">
            <div style="font-weight: 600; color: #1e40af; margin-bottom: 8px;">📋 自定义模型训练说明</div>
